...
/** \mainpage tree.hh
   ...
   2014.4.24 zhenglibao 替换range_error为assert(false)，因为
   在android平台下未定义range_error
*/
			...
			if(tmp->next_sibling==0) {
				...
				do {
					if(tmp==ret.top_node)
					   ...
					   //throw std::runtime_error("tree: begin_fixed out of range");
						   assert(false);
					tmp=tmp->parent;
               if(tmp==0) 
					   assert(false);//throw std::runtime_error("tree: begin_fixed out of range");
               --curdepth;
...
typename tree<T, tree_node_allocator>::fixed_depth_iterator tree<T, tree_node_allocator>::end_fixed(const iterator_base& pos, unsigned int dp) const
	...
	while(curdepth<dp) { // go down one level
		while(tmp->first_child==0) {
			tmp=tmp->next_sibling;
			if(tmp==0)
				assert(false); //throw std::runtime_error("tree: end_fixed out of range");
			}
		tmp=tmp->first_child;
		++curdepth;
		}
