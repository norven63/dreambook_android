...
namespace agg
{
    ...
    const float intersection_epsilon = 1.0e-20f;
	...
	AGG_INLINE float fast_sqrt_invert(float x) 
	{
		float xhalf = 0.5f*x;
		int i = *(int*)&x;
	...
	AGG_INLINE void normalize_point(float& x, float& y)
	{
		float len = fast_sqrt_invert(x*x + y*y);
		x *= len;
		y *= len;
	}
