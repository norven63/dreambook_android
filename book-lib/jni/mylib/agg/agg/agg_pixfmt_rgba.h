...
namespace agg
{
    ...
    {
    public:
		typedef pixfmt_alpha_blend_rgba<Blender,RenBuf,PixelT> self_type;
		...
		void invert_hspan(int x, int y,
			unsigned len)
		{
			value_type* p = (value_type*)m_rbuf->row_ptr(x, y, len) + (x << 2);
			do 
			{
				p[order_type::R] = base_mask-p[order_type::R];
				p[order_type::G] = base_mask-p[order_type::G];
				p[order_type::B] = base_mask-p[order_type::B];
				p += 4;
			}
			while(--len);
		}
                               ...
                               const int8u* covers)
        {
 		...
 		inline BYTE division_by_255(short number, short numerator)
 		{
		...
		// 
		void blend_lcd_solid_hspan(int x, int y,
			unsigned len, 
			const color_type& c,
			const int8u* covers)
		{
			if (c.a)
			{
				...
				{
					{
						p[order_type::B] = division_by_255(c.b,*(++covers));
						p[order_type::G] = division_by_255(c.g,*(++covers));
						p[order_type::R] = division_by_255(c.r,*(++covers));
						p[order_type::A] = c.a;
					}
		...
		//--------------------------------------------------------------------
		void copy_solid_hspan(int x, int y,
								unsigned len, 
								const color_type& c)
		{
			value_type* p = (value_type*)m_rbuf->row_ptr(x, y, len) + (x << 2);
		...
		//--------------------------------------------------------------------
		void copy_solid_vspan(int x, int y,
								unsigned len, 
								const color_type& c)
		{
			do 
			...
			unsigned nSourceWidth)
		{
			...
			do 
			{
				p[order_type::R] = colors[n].r;
				p[order_type::G] = colors[n].g;
				p[order_type::B] = colors[n].b;
				p[order_type::A] = colors[n].a;
				++x;
				n = x%nSourceWidth;
				p += 4;
			}
    ...
    {
    public:
		typedef pixfmt_custom_blend_rgba<Blender,RenBuf> self_type;
