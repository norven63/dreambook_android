...
namespace agg
{
    ...
    {
    public:
		typedef renderer_mclip<PixelFormat> self_type;
		...
		// modified at 2012.12.28
		renderer_mclip() : m_curr_cb(0) {}
		...
		unsigned get_clip_count() const
		{
			return m_clip.size();
		}
		void get_clip_at(int nindex,rect_i& rcClip)
		{
			if(nindex>=0 && nindex<m_clip.size())
			{
				rcClip=m_clip[nindex];
			}
		}
                       ...
                       int y_to=0)
        {
            ...
            do
            {
                m_ren.copy_from(from.m_ren, rc, x_to, y_to);
            }
            while(next_clip_box());
        }
