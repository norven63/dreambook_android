...
namespace agg
{
    ...
    public:
        ...
        {
            m_height = uceil((float)src.height());
            m_width  = uceil((float)src.width());
            m_width_hr = uround((float)src.width() * line_subpixel_scale);
            m_half_height_hr = uround((float)src.height() * line_subpixel_scale/2);
    ...
    public:
        ...
        {
            m_ren->blend_color_hspan(x, y, len, colors, 0, 255);
        }

        //-------------------------------------------------------------------------
        void blend_color_vspan(int x, int y, unsigned len, const color_type* colors)
        {
            m_ren->blend_color_vspan(x, y, len, colors, 0, 255);
        }
                ...
                if((flags & 4) == 0)
                {
                    if(flags)
                    {
                        line_parameters lp2(x1, y1, x2, y2, 
                                           uround(calc_distance((float)x1, (float)y1, (float)x2, (float)y2)));
                        if(flags & 1)
                        {
                            m_start += uround(calc_distance((float)lp.x1, (float)lp.y1, (float)x1, (float)y1) / m_scale_x);
                            sx = x1 + (y2 - y1); 
                            sy = y1 - (x2 - x1);
                        }
                        else
