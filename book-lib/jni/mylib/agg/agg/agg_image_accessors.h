...
namespace agg
{
    ...
    public:
        ...
        explicit image_accessor_clip(const pixfmt_type& pixf, 
            ...
            m_pixf(&pixf)
        {
            m_pixf->make_pix(m_bk_buf, bk); // @@@ changed for rgb_packed
        }
    ...
    public:
		...
		AGG_INLINE unsigned nWidth()
		{
			return m_wrap_x.GetSize();
		}
		AGG_INLINE int8u* GetYPix(int y)
		{
			return (m_pixf + y%m_nHeight * m_nWidth* pix_width);
		}

    private:
        pixfmt_type* m_pixf; // @@@ changed for invoke the member function of pixfmt_type -- rgb_packed
