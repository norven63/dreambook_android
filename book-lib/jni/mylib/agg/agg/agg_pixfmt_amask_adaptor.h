...
namespace agg
{
    ...
    {
    public:
		typedef pixfmt_amask_adaptor<PixFmt,AlphaMask> self_type;
    ...
    public:
       ...
       void copy_from(const self_type& from, 
                       ...
                       unsigned len)
        {
			const color_type* p=(const color_type*)from.m_pixf->row_ptr(xsrc,ysrc,len);
			copy_color_hspan(xdst,ydst,len,p);
            //m_pixf->copy_from(*from.m_pixf, xdst, ydst, xsrc, ysrc, len);
        }
		...
		void blend_from(const SrcPixelFormatRenderer& from, 
			...
			int8u cover)
		{
			const color_type* p=(const color_type*)from.row_ptr(xsrc,ysrc,len);
			blend_color_hspan(xdst,ydst,len,p,NULL);
			//m_pixf->blend_from(from,xdst,ydst,xsrc,ysrc,len,cover);
		}


        //--------------------------------------------------------------------
		// added at 2012.12.28 by zhenglibao
		void copy_solid_hspan(int x, int y,unsigned len,const color_type& c)
		{
			color_type ct(c);
			ct.a = color_type::base_mask;
			copy_hline(x,y,len,ct);
		}
		//--------------------------------------------------------------------
		// added at 2012.12.28 by zhenglibao
