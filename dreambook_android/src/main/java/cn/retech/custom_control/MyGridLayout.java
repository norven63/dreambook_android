...
/**
 * 此类是从Android系统中copy过来的
 * 
 * This is a fully backwards-compatible version of MyGridLayout, which works all
 * the way back to Android 1.5.
 * 
 * IMPORTANT: There is one difference between this MyGridLayout and the one the
 * default. When you change the visibility of a child View, you must also call
 * MyGridLayout.notifyChildVisibilityChanged(). This workaround exists because
...
public class MyGridLayout extends ViewGroup {
	 ...
	 * <p>
	 * For example, when {@code MyGridLayout} is in {@link #ALIGN_BOUNDS} mode,
	 ...
	 * <p>
	 * For example, when {@code MyGridLayout} is in {@link #ALIGN_MARGINS} mode,
	...
	static final String TAG = MyGridLayout.class.getName();
	...
	private static final int ORIENTATION = R.styleable.MyGridLayout_android_orientation;
	private static final int ROW_COUNT = R.styleable.MyGridLayout_rowCount;
	private static final int COLUMN_COUNT = R.styleable.MyGridLayout_columnCount;
	private static final int USE_DEFAULT_MARGINS = R.styleable.MyGridLayout_useDefaultMargins;
	private static final int ALIGNMENT_MODE = R.styleable.MyGridLayout_alignmentMode;
	private static final int ROW_ORDER_PRESERVED = R.styleable.MyGridLayout_rowOrderPreserved;
	private static final int COLUMN_ORDER_PRESERVED = R.styleable.MyGridLayout_columnOrderPreserved;
	...
	/**
	 * Returns whether or not this MyGridLayout will allocate default margins
	 * when no corresponding layout parameters are defined.
	 * 
	...
	/**
	 * When {@code true}, MyGridLayout allocates default margins around children
	 ...
	 *            use {@code true} to make MyGridLayout allocate default margins
	 * 
	...
	/**
	 * When this property is {@code true}, MyGridLayout is forced to place the
	 * row boundaries so that their associated grid indices are in ascending
	 * order in the view.
	 * <p>
	 * When this property is {@code false} MyGridLayout is at liberty to place
	 * the vertical row boundaries in whatever order best fits the given
	 * constraints.
	 * <p>
	 * The default value of this property is {@code true}.
	 * 
	 * @param rowOrderPreserved
	 *            {@code true} to force MyGridLayout to respect the order of row
	 *            boundaries
	 * 
	 ...
	 * <p>
	 * When this property is {@code false} MyGridLayout is at liberty to place
	 * the horizontal column boundaries in whatever order best fits the given
	 * constraints.
	 * <p>
	 * The default value of this property is {@code true}.
	 * 
	 * @param columnOrderPreserved
	 *            use {@code true} to force MyGridLayout to respect the order of
	 *            column boundaries.
	 * 
	...
	protected void onDraw(Canvas canvas) {
		...
		if (DEBUG) {
			...
			for (int i = 0; i < getChildCount(); i++) {
				View c = getChildAt(i);
				drawRect(canvas, c.getLeft() - getMargin1(c, true, true), c.getTop() - getMargin1(c, false, true), c.getRight() + getMargin1(c, true, false),
	...
	private void measureChildWithMargins2(View child, int parentWidthSpec, int parentHeightSpec, int childWidth, int childHeight) {
		int childWidthSpec = getChildMeasureSpec(parentWidthSpec, getPaddingLeft() + getPaddingRight() + getTotalMargin(child, true), childWidth);
		int childHeightSpec = getChildMeasureSpec(parentHeightSpec, getPaddingTop() + getPaddingBottom() + getTotalMargin(child, false), childHeight);
	 ...
	 * grid lines; along either the horizontal or vertical axis. A MyGridLayout
	...
	final class Axis {
		...
		private void computeGroupBounds() {
			...
			for (int i = 0, N = getChildCount(); i < N; i++) {
				...
				LayoutParams lp = getLayoutParams(c);
				Spec spec = horizontal ? lp.columnSpec : lp.rowSpec;
				groupBounds.getValue(i).include(c, spec, MyGridLayout.this, this);
			}
		}
		...
		private void include(List<Arc> arcs, Interval key, MutableInt size, boolean ignoreIfAlreadyPresent) {
			 ...
			 * constraints when MyGridLayout has no children . when components
		...
		private void logError(String axisName, Arc[] arcs, boolean[] culprits0) {
			...
			Log.d(TAG, axisName + " constraints: " + arcsToString(culprits) + " are inconsistent; " + "permanently removing: " + arcsToString(removed) + ". ");
		}
		 ...
		 * O(N^2) to O(N)
		 * 
		 * MyGridLayout converts its requirements into a system of linear
	...
	/**
	 * Layout information associated with each of the children of a
	 * MyGridLayout.
	 * <p>
	 * MyGridLayout supports both row and column spanning and arbitrary forms of
	 ...
	 * layout parameters. {@link android.widget.MyGridLayout.Spec Specs} are
	 ...
	 * {@link MyGridLayout.LayoutParams#setGravity(int)} method.
	 * 
	 ...
	 * explicitly declared in the layout parameters of MyGridLayout's children.
	 * In addition, MyGridLayout does not distinguish the special size value
	 ...
	 * {@link MyGridLayout#setUseDefaultMargins(boolean) useDefaultMargins} is
	 ...
	 * </ul>
	 * 
	 * See {@link MyGridLayout} for a more complete description of the
	 * conventions used by MyGridLayout in the interpretation of the properties
	 * of this class.
	 * 
	 ...
	 * 
	 * This is useful in the MyGridLayout class where we can rely on the order
	 * of children not changing during layout - to use integer-based lookup for
	 * our internal structures rather than using (and storing) an implementation
	 * of Map<Key, ?>.
	 */
	@SuppressWarnings(value = "unchecked")
	...
	static class Bounds {
		...
		protected final void include(View c, Spec spec, MyGridLayout myGridLayout, Axis axis) {
			this.flexibility &= spec.getFlexibility();
			int size = myGridLayout.getMeasurementIncludingMargin(c, axis.horizontal);
			Alignment alignment = myGridLayout.getAlignment(spec.alignment, axis.horizontal);
	 ...
	 * group. See {@link MyGridLayout} for a description of the conventions used
	 * by MyGridLayout for grid indices.
	 * <p>
	...
	public static class Spec {
		static final Spec UNDEFINED = spec(MyGridLayout.UNDEFINED);
	 ...
	 * independently.
	 * <p>
	 * The MyGridLayout class defines the most common alignments used in general
	...
	public static abstract class Alignment {
		 ...
		 *            This parameter is currently unused as MyGridLayout only
