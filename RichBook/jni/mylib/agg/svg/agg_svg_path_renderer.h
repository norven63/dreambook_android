...
namespace svg
{
    ...
    struct path_attributes
    {
		...
		int										gradient_flag; // 0:no, 1:horizontal, 2:vertical
		std::vector<std::pair<float, rgba8> >	gradient_color;
        ...
        path_attributes(const path_attributes& attr) :
			...
			gradient_color(attr.gradient_color),
    ...
    public:
		...
		void SetSign(bool bSign = false) { m_bSign = bSign; m_fSignTop = 1024.0f; }
		float GetSignTop() { return m_fSignTop; }
        void remove_all();
        ...
        void move_to(float x, float y, bool rel=false);   // M, m
        void line_to(float x,  float y, bool rel=false);  // L, l
        void hline_to(float x, bool rel=false);            // H, h
        void vline_to(float y, bool rel=false);            // V, v
        void curve3(float x1, float y1,                   // Q, q
                    float x,  float y, bool rel=false);
        void curve3(float x, float y, bool rel=false);    // T, t
        void curve4(float x1, float y1,                   // C, c
                    float x2, float y2, 
                    float x,  float y, bool rel=false);
        void curve4(float x2, float y2,                   // S, s
                    float x,  float y, bool rel=false);
		void ellip(float x, float y, float rx, float ry);
		void round_rectangle(float l, float t, float r, float b, float rx, float ry);
...
//        }
        ...
        void fill_none();
        void stroke_none();
        void fill_opacity(float op);
        void stroke_opacity(float op);
        ...
        void bounding_rect(float* x1, float* y1, float* x2, float* y2)
        {	
		...
		path_storage& get_path() { return m_storage; }
                    ...
                    float opacity=1.0f)
        {
            ...
            ras.clip_box(float(cb.x1), float(cb.y1), float(cb.x2), float(cb.y2));
            ...
            for(i = 0; i < m_attr_storage.size(); i++)
            {
                ...
                if(attr.fill_flag)
                {
                    ...
                    if(fabs(m_curved_trans_contour.width()) < 0.0001f)
                    {
					...
					if (m_bSign)
					{
						...
						if (m_fSignTop > (float)ras.min_y())
							m_fSignTop = (float)ras.min_y();
					}
					else
