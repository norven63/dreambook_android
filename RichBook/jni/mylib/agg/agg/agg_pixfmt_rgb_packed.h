...
namespace agg
{
    ...
    struct blender_rgb555
    {
        ...
        typedef int16u pixel_type;
		 typedef order_rgb order_type; // @@@ changed for add color heft order
    ...
    struct blender_bgrAAA
    {
        ...
        typedef color_type::calc_type calc_type;
        typedef int32u pixel_type;
		 typedef order_bgr order_type; // @@@ changed for add color heft order
    ...
    {
    public:
		typedef pixfmt_alpha_blend_rgb_packed<Blender,RenBuf> self_type;
        ...
        typedef typename blender_type::pixel_type pixel_type;
		 typedef typename blender_type::order_type order_type; // @@@ changed for rgb555 and rgb565, others may not be right
    ...
    public:
		...
		AGG_INLINE		 int8u* row_ptr(int x,int y,unsigned len)		{ return m_rbuf->row_ptr(x,y,len); }
		AGG_INLINE const int8u* row_ptr(int x,int y,unsigned len) const { return m_rbuf->row_ptr(x,y,len); }
        ...
        //--------------------------------------------------------------------
        AGG_INLINE void make_pix(int8u* p, const color_type& c) const // @@@ changed for not changing the members by adding const
        {
		...
		AGG_INLINE void clear(BYTE& bclr)
		{
			unsigned w = width();
			unsigned h = height();
			unsigned total = w * h * 3;
			unsigned char* pBuf = (unsigned char*)(m_rbuf->row_ptr(0, 0, 0));
			memset((void*)pBuf, bclr, total);
		}
		...
		//--------------------------------------------------------------------
		void copy_solid_hspan(int x, int y, unsigned len, const color_type& c)
		{
			pixel_type* p = (pixel_type*)m_rbuf->row_ptr(x, y, len) + x;
			do 
			{
		...
		//--------------------------------------------------------------------
		void copy_solid_vspan(int x, int y, unsigned len, const color_type& c)
		{
			do 
			{
				pixel_type* p = (pixel_type*)m_rbuf->row_ptr(x, y++, 1) + x;
		...
		void copy_color_hspan_rep(int x, int y,unsigned len,
			const color_type* colors,
			unsigned nSourceWidth)
		{
			pixel_type* p = (pixel_type*)m_rbuf->row_ptr(x, y, len) + x;
			int n = x % nSourceWidth;
			do 
			{				
				*p++ = m_blender.make_pix(colors[n].r, colors[n].g, colors[n].b);
				++x;
				n = x % nSourceWidth;
			}
			while(--len);
