...
namespace agg
{
                                   ...
                                   BaseRenderer& ren, const ColorT& color)
    {
	 ...
	 //================================================render_scanline_aa_solid
	 template<class Scanline, class BaseRenderer, class ColorT> 
	 void render_scanline_no_aa_solid(const Scanline& sl, 
									 BaseRenderer& ren, 
									 const ColorT& color)
	 {
		 int y = sl.y();
		 ...
		 for(;;)
		 {
			 ...
			 if(span->len > 0)
			 {
				 ren.copy_solid_hspan(x, y, (unsigned)span->len, color);
			 }
			 else
			 {
				 ren.copy_hline(x, y, (unsigned)(x - span->len - 1), color);
			 }
			 if(--num_spans == 0) break;
			 ++span;
		 }
	 ...
	 //===============================================render_scanlines_no_aa_solid
	 template<class Rasterizer, class Scanline, 
				 class BaseRenderer, class ColorT>
	 void render_scanlines_no_aa_solid(Rasterizer& ras, Scanline& sl, 
										BaseRenderer& ren, const ColorT& color)
	 {
		 if(ras.rewind_scanlines())
		 {
			 ...
			 while(ras.sweep_scanline(sl))
			 {
				 ...
				 for(;;)
				 {
					 ...
					 if(span->len > 0)
					 {
						 ren.copy_solid_hspan(x, y, (unsigned)span->len, 
							 ren_color);
					 }
					 else
					 {
						 ren.copy_hline(x, y, (unsigned)(x - span->len - 1), 
							 ren_color);
					 }
					 if(--num_spans == 0) break;
					 ++span;
		 ...
		 //typename BaseRenderer::color_type ren_color(color);
		 //ren_color.r = 124, ren_color.g = 255, ren_color.b = 255;
		 //for (int i =0; i < ras.total_cells(); i++)
		 //{
		 // cell_no_aa* pCell = ras.get_cell(i);
		 // ren.copy_pixel(pCell->x, pCell->y, ren_color);
		 //}
		 return;
	 }
	...
	//==============================================renderer_scanline_no_aa_solid
	template<class BaseRenderer> class renderer_scanline_no_aa_solid
	{
	public:
		typedef BaseRenderer base_ren_type;
		typedef typename base_ren_type::color_type color_type;
		...
		//--------------------------------------------------------------------
		renderer_scanline_no_aa_solid() : m_ren(0) {}
		explicit renderer_scanline_no_aa_solid(base_ren_type& ren) : m_ren(&ren) {}
		void attach(base_ren_type& ren)
		{
			m_ren = &ren;
		}
		...
		template<class Scanline> void render(const Scanline& sl)
		{
			render_scanline_no_aa_solid(sl, *m_ren, m_color);
		}
                             ...
                             SpanAllocator& alloc, SpanGenerator& span_gen)
    {
	 ...
	 template<class Scanline, class BaseRenderer, 
	 class SpanAllocator, class SpanGenerator> 
		 void render_scanline_no_aa(const Scanline& sl, BaseRenderer& ren, 
		 SpanAllocator& alloc, SpanGenerator& span_gen)
	 {
		 int y = sl.y();
		 int nWidth = span_gen.GetWidth();
		 typename BaseRenderer::color_type* colors = alloc.allocate(nWidth);
		  //span_gen.GetYSourceColor(colors,y);
		  span_gen.generate(colors, 0, y, nWidth);
		 ...
		 for(;;)
		 {
			 ...
			 /*typename BaseRenderer::color_type* colors = alloc.allocate(len);
			 span_gen.generate(colors, x, y, len);
			 ren.copy_color_hspan(x, y, len, colors);*/
			 ren.copy_color_hspan_repeat(x, y, len, colors,nWidth);
			 if(--num_spans == 0) break;
			 ++span;
		 }
	 }
	//=====================================================render_scanlines_no_aa
	 template<class Rasterizer, class Scanline, class BaseRenderer, 
	 class SpanAllocator, class SpanGenerator>
		 void render_scanlines_no_aa(Rasterizer& ras, Scanline& sl, BaseRenderer& ren, 
		 SpanAllocator& alloc, SpanGenerator& span_gen)
	 {
		 if(ras.rewind_scanlines())
		 {
			 ...
			 while(ras.sweep_scanline(sl))
			 {
				 render_scanline_no_aa(sl, ren, alloc, span_gen);
			 }
		 }
	 }
