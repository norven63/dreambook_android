package com.retech.richbook.activity;
...
import com.retech.richbook.R;
import com.retech.richbook.customView.BookContentView;
import com.retech.richbook.customView.BookContentView.OperateBookListener;
import com.retech.richbook.reader.BookReader;
import com.retech.richbook.reader.BookReader.BookOperation;
import com.retech.richbook.reader.BookReader.FontSizeOpterationEnum;
import com.retech.richbook.reader.CRBClickListener;
import com.retech.richbook.reader.CalcLayoutListener;
...
public class RichTextReaderActivity extends Activity {
	...
	private BookContentView bookContentView;// 渲染书籍的容器
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bookContentView = (BookContentView) findViewById(R.id.showBookImageView);
		bookContentView.setOperateBookListener(new OperateBookListener() {
			@Override
			public void onUp(float x, float y) {
				BookReader.getInstance.onClick(x, y);

				if (!bookContentView.hasUpOrMove()) {
	...
	public void onWindowFocusChanged(boolean hasFocus) {
		...
		if (!BookReader.getInstance.prepareToWork(bookContentView, crbClickListener, calcLayoutListener, bookPath)) {
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_MENU:
			...
			if (bookContentView.isOperatingBook()) {
				hiddenOpterationLayout();
			} else {
	...
	private void showOpterationLayout() {
		bookContentView.setIsOperatingBook(true);

		pageNumberLayout.animate().y(0f).setDuration(350);
		opterationLayout.animate().y((Float) opterationLayout.getTag()).setDuration(350);
	}
	...
	private void hiddenOpterationLayout() {
		bookContentView.setIsOperatingBook(false);

		pageNumberLayout.animate().y(-pageNumberLayout.getHeight()).setDuration(350);
		opterationLayout.animate().y(opterationLayout.getY() + opterationLayout.getHeight()).setDuration(350);
